...
/**
 * made to run local on visl machine
 * 
 * @author hfernandes
 *
 */
public class POSTaggerLocal {
	...
	private static void VISLTagger(String input, String output) throws Exception{
		...
		CommandLine cmdLine;
		File run = new File("bin/visl-local.sh");
		...
		for (Comment comment : corpus.getComments()) {
		   for (Sentence sentence : comment.getSentences()) {
			   ...
			   VISLParser.processSentence4(sentence); // process the sentence tokens
			   baos.reset();
			   tmpOS.close();
		   }
	...
	private static void VISLReTagger(String input, String output) throws Exception{
		// read input file
		Serializer serializer = new Persister();
		File inputFile = new File(input);
		Corpus corpus = serializer.read(Corpus.class, inputFile);
		
		System.out.println("ReTagging "+input+"...");
		...
		for (Comment comment : corpus.getComments()) {
		   for (Sentence sentence : comment.getSentences()) {
			   ...
			   sentence.setClauses(new ArrayList<Clause>());
			   VISLParser.processSentence4(sentence); // process the sentence tokens
		   }
...
//		VISLTagger(args[0], args[1]);
		
		VISLReTagger("output/Sentituites_pos_5.xml", "output/Sentituites_pos_5_.xml");
	}
