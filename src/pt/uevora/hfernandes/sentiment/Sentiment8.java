...
//						}
						...
						double tokenPol = getPolarity(token, invertPolarity, increasePolarity, decreasePolarity);
						int distance = token.getIndex() > sentence.getAlvoToken() ? token.getIndex() - sentence.getAlvoToken() : sentence.getAlvoToken()-token.getIndex();
						
						if(distance > 4){
							tokenPol = tokenPol * (1/distance);
							polarity += getPolarity(token, invertPolarity, increasePolarity, decreasePolarity)*tokenPol;
						}
						else{
...
//		String OUTPUT = "output/SentiCorpus-PT_sentiment_8.xml";
		String OUTPUT = "output/Sentituites_sentiment_8.xml";
//		String eval = "output/SentiCorpus-PT_sentiment_match_7.txt";
		
		Sentiment8 sent = new Sentiment8();
		
		sent.evaluate(LEXICON, INPUT, OUTPUT, null);
//		new Evaluation2().evaluate9();
		new Evaluation2().evaluate9Sentituites();
	}
}
